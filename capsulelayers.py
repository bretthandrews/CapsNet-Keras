"""
Some key layers used for constructing a Capsule Network. These layers
can used to construct CapsNet on other dataset, not just on MNIST.
*NOTE*: some functions can be implemented in multiple ways, I keep all
of them. You can try them for yourself just by uncommenting them and
commenting their counterparts.
...
class Length(layers.Layer):
    ...
    that has the same shape with y_true in margin_loss. Using this
    layer as model's output can directly predict labels by using
...
class Mask(layers.Layer):
    ...
    For example:
        ...
        # True labels. 8 samples, 3 classes, one-hot coding.
        y = keras.layers.Input(shape=[8, 3])

        out = Mask()(x)  # out.shape=[8, 6]
        # or
        # out2.shape=[8,6]. Masked with true labels y.
...
def squash(vectors, axis=-1):
    """
    The non-linear activation used in Capsule. It drives the length of
    a large vector to near 1 and small vector to 0.
...
class CapsuleLayer(layers.Layer):
    ...
    `in_num` inputs, each is a scalar, the output of the neuron from
    the former layer, and it has `out_num` output neurons. CapsuleLayer
    just expand the output of the neuron from scalar to vector. So its
    ...
    :param dim_capsule: dimension of the output vectors of the capsules
        in this layer
